...
把「⚖ 分析合理性」的結果從「頂部一份純文字清單」改造成「一張可掃視的地圖」:
- **判定帶**:依 verdict(妥當/有疑慮/有問題)著色的左邊條 + 總評 + 嚴重度**堆疊條**;整體性(step 0)指摘就掛在帶內。
- **步驟健康條**:每步一格,被指摘的步驟依最高嚴重度上色 + `!` 標記;點格子跳到並高亮該步。有 step>0 指摘時才顯示。
- **指摘就地內嵌**:每筆指摘直接掛在它所指的工作項目下方(嚴重度 + 分類 + 問題 + 建議),不再與時間軸兩地對照。
- **新增 `category` 分類**:`/analyze` schema 加 `category`(危險操作 / 多餘步驟 / 遺漏 / 更好做法 / 其他),`ANALYSIS_PROMPT_VERSION` bump `v1`→`v2`,`normalizeFinding` 夾限未知值為 `other`;匯出記錄自動帶上。
- 退役舊 `AnalysisPanel`。後端測試 133 → 136、前端測試 83 → 86。
